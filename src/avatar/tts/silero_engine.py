...
class SileroEngine(ITTSEngine):
    def __init__(
        self, language: str = "ru", speaker: str = "xenia", sample_rate: int = 48000, device: str = "cpu"
    ) -> None:
        ...
        if self._model is None:
            ...
            if not model_path.exists():
                ...
                try:
                    ...
                    with (
                        Path(model_path).open("wb") as file,
                        tqdm(
                            desc="Silero Model",
                            total=total_size,
                            unit="iB",
                    ...
                    raise RuntimeError(
                        f"Could not download Silero model. Please download manually from {url} to {model_path}"
                    ) from e
            ...
            try:
                self._model = torch.package.PackageImporter(model_path).load_pickle("tts_models", "model")  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def synthesize(self, text: str, language: str = "ru", speaker_wav: str | None = None) -> AudioSegment:
        if not text:
        ...
        audio_tensor = model.apply_tts(text=text, speaker=self.speaker, sample_rate=self.sample_rate)
        ...
        return AudioSegment(audio_bytes=audio_bytes, sample_rate=self.sample_rate, format="wav", duration=duration)

    async def synthesize_streaming(self, text: str, language: str = "ru") -> AsyncIterator[AudioSegment]:
