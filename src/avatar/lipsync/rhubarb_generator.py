...
class RhubarbGenerator(ILipSyncGenerator):
    ...
    async def generate_blendshapes(
        ...
        """
        if not audio_path.exists():  # noqa: ASYNC240
    ...
    def _run_rhubarb(self, audio_path: Path, recognizer: str) -> dict:
        ...
        try:
            ...
            logger.debug("Rhubarb output parsed successfully")
            return rhubarb_json
            ...
            msg = "Rhubarb CLI timed out after 300s"
            logger.error(msg)
            raise RuntimeError(msg) from e
    ...
    def _parse_rhubarb_output(self, rhubarb_json: dict) -> BlendshapeWeights:
        ...
        for cue in mouth_cues:
            ...
            frame = BlendshapeFrame(timestamp=timestamp, mouth_shapes={blendshape_name: 1.0})
            frames.append(frame)

        logger.debug(f"Parsed {len(frames)} frames from Rhubarb output")

        return BlendshapeWeights(frames=frames, fps=30, duration=duration)
